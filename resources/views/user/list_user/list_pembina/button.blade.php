...
<form class="delete-form" action="{{ route('pembina_list.destroy', $id) }}" method="POST">
    @csrf
    @method('DELETE')
    <div class="d-flex gap-3">

        @if (Auth::user()->roles == 'Administrator')
            <a href="{{ route('pembina_list.edit', $id) }}" class="text-success" data-toggle="tooltip" data-placement="top"
                title="Edit Data"><i class="mdi mdi-pencil font-size-18"></i></a>
        ...
        $('.reset_confirm').click(function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('pembina_list.reset_password_pembina', ['id' => '__id__']) }}'
                        ...
                        success: function(data) {
                            ...
                            Swal.fire(
                                'Reset Berhasil!',
                                'Format (NIS+tgl+Bln), ex :12342508',
                                'success'
                            );
                        },
